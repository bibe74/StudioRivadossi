...
/**
 * @function FXML.sfn_SplitString
 * @description

 * @param_input @input
 * @param_input @splitter
*/

CREATE OR ALTER FUNCTION FXML.sfn_SplitString (
    @input VARCHAR(MAX),
    @splitter VARCHAR(99)
) RETURNS TABLE
AS
...
/**
 * @function FXML.sfn_GeneraScontoMaggiorazione
 * @description

 * @param_input @Sconto
*/

CREATE OR ALTER FUNCTION FXML.sfn_GeneraScontoMaggiorazione (
    @Sconto NVARCHAR(10),
...
) S CROSS APPLY FXML.sfn_GeneraScontoMaggiorazione (S.Sconto, 100.0) T;
GO

/**
 * @function FXML.sfn_PercentualeScontoMaggiorazione
 * @description 

 * @param_input @Sconto
*/

CREATE OR ALTER FUNCTION FXML.sfn_PercentualeScontoMaggiorazione (
    @Sconto NVARCHAR(10)
)
...
SELECT
    *,
    FXML.sfn_PercentualeScontoMaggiorazione(S.Sconto) AS PercentualeSconto
FROM (
...
/**
 * @stored_procedure FXML.ssp_EsportaDatiFattura_Header
 * @description
...
CREATE OR ALTER PROCEDURE FXML.ssp_EsportaDatiFattura_Header (
...
BEGIN
	...
	SET FEH.DataOraInserimento = CURRENT_TIMESTAMP,
		...
		FEH.CessionarioCommittente_Sede_Provincia = CASE WHEN COALESCE(NC.SDI_IDNazione, N'') = N'IT' THEN C.Provincia ELSE N'-1' END, -- 1.4.2.5
...
/**
 * @stored_procedure FXML.ssp_EsportaDatiFattura_Body
 * @description
...
CREATE OR ALTER PROCEDURE FXML.ssp_EsportaDatiFattura_Body (
	@IDDocumento UNIQUEIDENTIFIER,
	@PKStaging_FatturaElettronicaHeader BIGINT
) AS
BEGIN
